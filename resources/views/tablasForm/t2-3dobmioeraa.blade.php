...
@section('content')
	...
	<div class="row">
	     <div id="panelTitu" class="panel-heading text-center separar"><h5 id="h5Titu"><b>ANEXO T-2-3 / DATOS DE LOS ORIGENES (FORMAS DE ADQUISICIÓN) DE LOS BIENES MUEBLES E INMUEBLES DEL ORGANO O ENTE</b></h5></div>
	</div>


	<div class="row separar">
		<div class="col-md-12">
			<h7><b>D) Aplicable solo para la forma de adquisición de Dación en Pago.</b></h7>
		</div>
	</div>
...
<div class="row">
   <div class="col-md-12">
   	  <form role="form" id="formValidaT23" name="formValidaT23" method="POST" action="{{url('t23')}}">
    	{{ csrf_field() }}
    ...
    <!--ARRAY OF INPUT-TEXT arrayT23 BELONGING TO CONTROLADORT23, TABLE RELATED IN THE BD => T23-->
      ...
      <div class="col-md-4 form-group {{$arrayT23[$posicion][4]}} separar">
            <label for="{{$arrayT23[$posicion][0]}}">{{$arrayT23[$posicion][1]}}</label>
              <input type="text" name="{{$arrayT23[$posicion][0]}}" id="{{$arrayT23[$posicion][0]}}" class="form-control" placeholder="{{$arrayT23[$posicion][2]}}"  >
          </div>
      @endforeach
