...
@section('content')
  ...
  <div class="row">
      <div class="panel-heading text-center separar"><h5><b>ANEXO T-4 / DATOS DE LOS RESPONSABLES DE LOS BIENES MUEBLES E INMUEBLES DEL ORGANO O ENTE</b></h5></div>
  </div>
...
<div class="row">
   <div class="col-md-12">
      <form role="form" id="formValidaT4" name="formValidaT4" method="POST" action="{{url('t4')}}">
      {{ csrf_field() }}
      ...
      <div class="col-md-4 form-group separar">
        <label for="{{$select2[$posicion][0]}}">{{$select2[$posicion][1]}}</label>
            <select name="{{$select2[$posicion][0]}}" id="{{$select2[$posicion][0]}}" class="form-control">
                <option value="0" disabled selected>Seleccione</option>
              @foreach($infoSelect41 as $traeSelect)
               <option value="{{$traeSelect->id}}">{{$traeSelect->opcion}}</option> 
              @endforeach
            </select>
      </div>
