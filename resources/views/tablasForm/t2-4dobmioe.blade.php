...
@section('content')
	...
	<div class="row">
	     <div id="panelTitu" class="panel-heading text-center separar"><h5 id="h5Titu"><b>ANEXO T-2-4 / DATOS DE LOS ORIGENES (FORMAS DE ADQUISICIÓN) DE LOS BIENES MUEBLES E INMUEBLES DEL ORGANO O ENTE</b></h5></div>
	</div>
...
<div class="row">
   <div class="col-md-12">
   	  <form role="form" id="formValidaT24" name="formValidaT24" method="POST" action="{{url('t24')}}">
    	{{ csrf_field() }}
    ...
    <!--ARRAY OF INPUT-TEXT arrayT24 BELONGING TO CONTROLADORT24, TABLE RELATED IN THE BD => T24-->
            ...
            <label for="{{$arrayT24[$posicion][0]}}">{{$arrayT24[$posicion][1]}}</label>
              <input type="text" name="{{$arrayT24[$posicion][0]}}" id="{{$arrayT24[$posicion][0]}}" class="form-control" placeholder="{{$arrayT24[$posicion][2]}}" maxlength="{{$arrayT24[$posicion][3]}}">
          </div>
